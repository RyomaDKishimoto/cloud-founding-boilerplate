...
import { GiftType } from "@/data/siteMetadata";

type Ts = Omit<GiftType, "href" | "quantity"> & {
  onSelect: (gift: any) => void;
};

const HorizontalCard: NextPage<Ts> = ({
  title,
  description,
  imgSrc,
  price,
  ...
  return (
    <button
      type="button"
      onClick={() => {
        onSelect((prev: { apiId: string; selected: boolean }[]) =>
          prev.map((item: { apiId: string; selected: boolean }) => {
